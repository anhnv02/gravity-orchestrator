    ...
    public async logoutAccount(email?: string): Promise<boolean> {
        ...
        if (email) {
            ...
            if (activeEmail === email) {
                // TokenStorage.clearTokenForAccount already switched activeAccount
                const newActiveEmail = await this.tokenStorage.getActiveAccount();
                if (newActiveEmail) {
                    const token = await this.tokenStorage.getTokenForAccount(newActiveEmail);
                    this.userEmail = token?.email || newActiveEmail;
                    this.setState(AuthState.AUTHENTICATED);
                } else {
    ...
    public async switchAccount(email: string): Promise<boolean> {
        ...
        try {
            ...
            if (isApiReady) {
                ...
                // Switch using API
                const switchResponse = await GravityOrchestratorApi.switchAccount(account.id);
