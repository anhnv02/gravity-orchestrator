 ...
 * Default port: 19527
 */
export class AntigravityToolsApi {
    ...
    public static async isApiReady(port: number = this.DEFAULT_PORT): Promise<boolean> {
        try {
            ...
            logger.debug(`[AntigravityToolsApi] Health check failed on port ${port}:`, error);
            return false;
        }
    }
    ...
    private static makeRequest(
        ...
        return new Promise((resolve, reject) => {
            ...
            logger.info(`[AntigravityToolsApi] ${method} ${url}`);
                ...
                res.on('end', () => {
                    if (res.statusCode && res.statusCode >= 200 && res.statusCode < 300) {
                        try {
                            ...
                            logger.info(`[AntigravityToolsApi] Success: ${method} ${path}`);
                            resolve(response);
                        } catch (error) {
            ...
            req.on('error', (error) => {
                logger.error(`[AntigravityToolsApi] Request error: ${error.message}`);
                reject(error);
            });
