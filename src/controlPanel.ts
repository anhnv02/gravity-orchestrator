...
export class ControlPanel {
    ...
    private async handleMessage(message: WebviewMessage): Promise<void> {
        ...
        switch (message.command) {
            ...
            case 'showAccountInfo':
                if (message.email) {
                    vscode.window.showInformationMessage(`Account info: ${message.email}`);
                }
                break;

            case 'showAccountFingerprint':
                if (message.email) {
                    vscode.window.showInformationMessage(`Account fingerprint: ${message.email}`);
                }
                break;
            ...
            case 'downloadAccount':
                if (message.email) {
                    vscode.window.showInformationMessage(`Download account: ${message.email}`);
                }
                break;

            case 'viewAccount':
                if (message.email) {
                    vscode.window.showInformationMessage(`View account: ${message.email}`);
                }
                break;
        }
    }
    ...
    private async handleRefreshAccount(email: string, id?: string): Promise<void> {
        ...
        try {
            ...
            if (await AntigravityToolsApi.isApiReady()) {
                await AntigravityToolsApi.refreshAccountQuota(email, id);
    ...
    private async isApiReady(): Promise<boolean> {
        try {
            const isReady = await AntigravityToolsApi.isApiReady();
            if (isReady) {
    ...
    private async getQuotaFromLocalApi(): Promise<QuotaSnapshot | undefined> {
        try {
            ...
            const ideModels = this.filterIdeModels(quota.models);
            
            const models: ModelQuotaInfo[] = ideModels.map(model => {
            ...
            logger.info(`[ControlPanel] Filtered to ${models.length} IDE models (from ${quota.models.length} total)`);
    ...
    private filterIdeModels(models: any[]): any[] {
        ...
        return models.filter(model => {
            ...
            const isIdeModel = ideModelPatterns.some(pattern => pattern.test(normalizedName));

            if (!isIdeModel) {
                logger.debug(`[ControlPanel] Filtering out model: ${model.name} (normalized: ${normalizedName})`);
            }
            return isIdeModel;
        });
    }
...
${this.getModalStyles()}
    ...
    private mapModelForDisplay(m: any): { label: string; percent: number; colorClass: string; time: string } {
        ...
        if (m.name.includes('gemini-3-pro')) displayName = 'G3 Pro';
        else if (m.name.includes('gemini-3-flash')) displayName = 'G3 Flash';
        else if (m.name.includes('gemini-3-image')) displayName = 'G3 Image';
        else if (m.name.includes('claude')) displayName = 'Claude';
        else if (m.name.includes('gpt-oss')) displayName = 'GPT';
    ...
    private calculateRemainingTime(resetTime: any): string {
        if (!resetTime) return '2h';
        
        try {
            let reset: Date;
            if (typeof resetTime === 'number') reset = new Date(resetTime * 1000);
            else reset = new Date(resetTime);
