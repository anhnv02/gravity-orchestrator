...
import { filterModelsForDisplay } from './modelUtils';
...
export async function getMultiAccountInfo(): Promise<MultiAccountInfo> {
  try {
    ...
    const accountsInfo = await Promise.all(accountsResponse.accounts.map(async acc => {
      ...
      const rawTier = acc.quota?.subscription_tier || 'free';
      ...
      if (cached && (now - cached.timestamp < CACHE_TTL)) {
        ...
        if (acc.quota?.models) {
          try {
            ...
            const filteredModels = acc.quota.models.filter(model => filterModelsForDisplay(model.name));
      ...
      // Log for debugging
      logger.debug(`[AccountUtils] Account: ${acc.email}, disabled: ${acc.disabled}, proxy_disabled: ${acc.proxy_disabled}`);
